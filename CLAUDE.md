...
**Directory map:** `src/sections/` = full-width page scenes (Hero, Countdown, Story, Events, EventFilms, Venue, Rsvp, Footer, Envelope, ControlPanel). `src/components/` = reusable pieces (Petals, MusicToggle, ScratchCard, SongSearch, Icons). `src/styles/` = plain CSS; `base.css` `@import`s `tokens.css` (design tokens/CSS variables) and `fonts.css`, and `main.jsx` also imports `components.css`. Icons are keyed by string in `Icons.jsx` and referenced by name from event data.
...
- **Song search** (`components/SongSearch.jsx`) calls the public iTunes Search API directly from the browser (gated by `config.enableSongRequest`).
- **Media** lives in `public/` (`/videos/*.mp4` referenced by event `video` fields, `/assets/bg-music.mp3`, `og-image.jpg`).
